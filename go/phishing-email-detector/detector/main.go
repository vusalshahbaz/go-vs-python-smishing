...
type PhishingEmailDetector struct {
...
func NewPhishingEmailDetector() *PhishingEmailDetector {
	return &PhishingEmailDetector{}
}

func (d *PhishingEmailDetector) Fit(features [][]float64, labels []float64) {
	wg := sync.WaitGroup{}
	total := len(features)

	fmt.Printf("phishingEmailDetector.Fit() - total: %d, starting training...\n", total)

	linearModel := logistic.NewLogisticRegression(500, 0.01)
	wg.Add(1)
	go func() {
		fmt.Printf("phishingEmailDetector.Fit() - training LogisticRegression model...\n")
		linearModel.Fit(features, labels)
		fmt.Printf("phishingEmailDetector.Fit() - LogisticRegression model completed\n")
		wg.Done()
	}()
	...
	wg.Add(1)
	go func() {
		fmt.Printf("phishingEmailDetector.Fit() - training MultinomialNaiveBayes model...\n")
		mn.Fit(features, labels)
		fmt.Printf("phishingEmailDetector.Fit() - MultinomialNaiveBayes model completed\n")
		wg.Done()
	}()
	...
	wg.Add(1)
	go func() {
		fmt.Printf("phishingEmailDetector.Fit() - training DecisionTreeClassifier model...\n")
		dtree.Fit(features, labels)
		fmt.Printf("phishingEmailDetector.Fit() - DecisionTreeClassifier model completed\n")
		wg.Done()
	}()
	...
	wg.Wait()
	fmt.Printf("phishingEmailDetector.Fit() - all models trained, processed: %d\n", total)
}

func (d *PhishingEmailDetector) Predict(features [][]float64) []float64 {
	wg := sync.WaitGroup{}
	total := len(features)
	...
	for i := range features {
		...
		if (i+1)%100 == 0 || i+1 == total {
			fmt.Printf("\rphishingEmailDetector.Predict() - total: %d, processed: %d", total, i+1)
		}
	}
...
func (d *PhishingEmailDetector) Stats(predictions []float64, labels []float64) *Prediction {
...
func (d *PhishingEmailDetector) ensemble(predictions []float64) float64 {
