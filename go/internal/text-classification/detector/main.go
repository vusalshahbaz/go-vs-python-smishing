...
func (d *TextClassifier) Fit(features [][]float64, labels []float64) {
	wg := sync.WaitGroup{}
	total := len(features)

	fmt.Printf("textClassifier.Fit() - total: %d, starting training...\n", total)

	linearModel := logistic.NewLogisticRegression(500, 0.01)
	wg.Add(1)
	go func() {
		fmt.Printf("textClassifier.Fit() - training LogisticRegression model...\n")
		linearModel.Fit(features, labels)
		fmt.Printf("textClassifier.Fit() - LogisticRegression model completed\n")
		wg.Done()
	}()
	...
	wg.Add(1)
	go func() {
		fmt.Printf("textClassifier.Fit() - training MultinomialNaiveBayes model...\n")
		mn.Fit(features, labels)
		fmt.Printf("textClassifier.Fit() - MultinomialNaiveBayes model completed\n")
		wg.Done()
	}()
	...
	wg.Add(1)
	go func() {
		fmt.Printf("textClassifier.Fit() - training DecisionTreeClassifier model...\n")
		dtree.Fit(features, labels)
		fmt.Printf("textClassifier.Fit() - DecisionTreeClassifier model completed\n")
		wg.Done()
	}()
	...
	wg.Wait()
	fmt.Printf("textClassifier.Fit() - all models trained, processed: %d\n", total)
}

func (d *TextClassifier) Predict(features [][]float64) []float64 {
	wg := sync.WaitGroup{}
	total := len(features)
	...
	for i := range features {
		...
		if (i+1)%100 == 0 || i+1 == total {
			fmt.Printf("\rtextClassifier.Predict() - total: %d, processed: %d", total, i+1)
		}
	}
...
func (d *TextClassifier) Stats(predictions []float64, labels []float64) *Prediction {
...
func (d *TextClassifier) ensemble(predictions []float64) float64 {
